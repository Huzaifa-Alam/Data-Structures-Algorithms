...
void searchInBstloop(struct BinaryTreeNode* root, int search) {
    ...
    while (temp != NULL) {
        if (search == temp->data) {
            printf("Successfully found using loop approach...\n");
            return;
        }
        if (search < temp->data) {
            temp = temp->left;
    ...
    printf("Not found using loop approach...\n");
}
// Recursive function to search an element in the BST
struct BinaryTreeNode* searchInBstRecursive(struct BinaryTreeNode* root, int search) {
    // Base case: root is NULL or the search value is found
    if (root == NULL || root->data == search) {
        if (root != NULL) {
            printf("Successfully found using recursive approach...\n");
        } else {
            printf("Not found using recursive approach...\n");
        }
        return root;
    }

    // If the search value is less than the root's data, search in the left subtree
    if (search < root->data) {
        return searchInBstRecursive(root->left, search);
    }

    // If the search value is greater than the root's data, search in the right subtree
    return searchInBstRecursive(root->right, search);
}
